  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchFormations.fulfilled, (state, action) => {
        ...
        state.formations = action.payload.filter(formation => 
          formation && 
          formation.nomEcole && 
      ...
      .addCase(createFormation.fulfilled, (state, action) => {
        if (action.payload && action.payload.id) {
          ...
          const exists = state.formations.some(f => f.id === action.payload.id);
          if (!exists) {
      ...
      .addCase(updateLangue.fulfilled, (state, action) => {
        ...
        const index = state.langues.findIndex(l => l.id === action.payload.id);
        if (index !== -1) {
